...
class HyperPropagate(nn.Module):
    def __init__(self, H):
        ...
        self.act = nn.LeakyReLU(args.gat_alpha)

    def forward(self, x, S):
        hyper = torch.matmul(S.T, x)
        h = self.act(self.fc1(hyper.T).T) + hyper
        h = self.act(self.fc2(h.T).T)   + h
        h = self.act(self.fc3(h.T).T)   + h
        return self.act(torch.matmul(S, h))
...
class CoHGCLModel(nn.Module):
    def __init__(self, adj, tadj, trna_n, disease_n):
        ...
        self.thyper = nn.Parameter(init(torch.empty(D, H)))
        self.dhyper = nn.Parameter(init(torch.empty(D, H)))
        self.hg_t_layers = nn.ModuleList([HyperPropagate(H) for _ in range(3)])
        self.hg_d_layers = nn.ModuleList([HyperPropagate(H) for _ in range(3)])
